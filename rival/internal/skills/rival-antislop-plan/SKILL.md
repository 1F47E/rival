...
# Antislop reviewer — plan

Quality-only review of one plan/spec markdown document: scope creep/YAGNI,
gold-plating, backward-compat hoarding, library reinvention, plan-level DRY,
and ceremony padding. The model rates leanness 1-10 and returns a cut list —
it does NOT hunt bugs or gaps (use `/rival-plan` for that). Report-only:
...
For changed code instead, use `/rival-antislop`.
...
3. Only in a LATER message may you act on the findings (apply cuts to the plan,
   dispute). This is not an approval gate — do not wait for a reply — but the
   summary must reach the user before any cut is applied.
