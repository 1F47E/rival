...
Run Google Antigravity CLI (Gemini 3.5 Flash) via the `rival` Go binary. The run is detached and watched in the background — this skill does not block your session.
...
$ARGUMENTS
$DELIM
rival command antigravity --detach --workdir "$(pwd)" <"$RIVAL_IN" >"$RIVAL_OUT" 2>"$RIVAL_ERR"
rm -f "$RIVAL_IN"
